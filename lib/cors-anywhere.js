...
function isAllowedTargetHost(hostname, whitelist) {
  if (!whitelist.length) {
    return true;
  }
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    targetHostWhitelist: [],        // If non-empty, requests to hosts not in this list will be blocked.
  ...
  if (typeof corsAnywhere.targetHostWhitelist === 'string') {
    corsAnywhere.targetHostWhitelist = [corsAnywhere.targetHostWhitelist];
  }
  if (!Array.isArray(corsAnywhere.targetHostWhitelist)) {
    corsAnywhere.targetHostWhitelist = [];
  }
  corsAnywhere.targetHostWhitelist = corsAnywhere.targetHostWhitelist.map(function (hostPattern) {
  ...
  return function (req, res) {
    ...
    if (!isAllowedTargetHost(location.hostname, corsAnywhere.targetHostWhitelist)) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The target host "' + location.hostname + '" was not whitelisted by the operator of this proxy.');
      return;
    }
