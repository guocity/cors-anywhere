...
function hostMatchesPattern(hostname, pattern) {
  ...
  if (pattern.indexOf('*.') === 0) {
    ...
    return hostname.length > suffix.length && hostname.lastIndexOf(suffix) === hostname.length - suffix.length;
  }

  return hostname === pattern;
}

// Returns the site that initiated the request. Browsers omit Origin on subresource loads
// (<script>, <img>, <link>, no-cors fetch), so fall back to Referer, which is still sent.
function getRequestSource(req) {
  return req.headers.origin || req.headers.referer || '';
}

function isAllowedOrigin(source, whitelist) {
  if (!whitelist.length) {
    return true;
  }

  // Neither header present: not a browser page request, so there is no origin to whitelist.
  if (!source) {
    return true;
  }
  ...
  return whitelist.some(function (pattern) {
    return hostMatchesPattern(hostname, pattern);
  });
}
 ...
 * @return {object} URL parsed using URL
 */
function parseURL(req_url) {
  ...
  try {
    ...
    // Add 'path' property for compatibility with http-proxy and other parts of the code
...
/**
 * Extract a target URL from the query string, if present.
 *
 * Supports requests like: /?url=http://example.com
 *
 * @param req_url {string} The raw request URL, including query string.
 * @return {string|null} The target URL if present, otherwise null.
 */
function getUrlFromQuery(req_url) {
  var parsed;
  try {
    parsed = new URL(req_url, 'http://localhost');
  } catch (e) {
    return null;
  }
  var queryUrl = parsed.searchParams.get('url');
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originHostWhitelist: [],        // If non-empty, requests from origins not matching these hosts will be blocked.
  ...
  if (typeof corsAnywhere.originHostWhitelist === 'string') {
    corsAnywhere.originHostWhitelist = [corsAnywhere.originHostWhitelist];
  }
  if (!Array.isArray(corsAnywhere.originHostWhitelist)) {
    corsAnywhere.originHostWhitelist = [];
  }
  corsAnywhere.originHostWhitelist = corsAnywhere.originHostWhitelist.map(function (hostPattern) {
  ...
  return function (req, res) {
    ...
    if (!location) {
      var queryUrl = getUrlFromQuery(req.url);
      if (queryUrl) {
        location = parseURL(queryUrl);
      }
    }
    ...
    var requestSource = getRequestSource(req);
    if (!isAllowedOrigin(requestSource, corsAnywhere.originHostWhitelist)) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + requestSource + '" was not whitelisted by the operator of this proxy.');
      return;
    }
